...
class Vehicle {
  ...
  public:
    ...
    void wander(const ofVec2f & target) {
      float angle = ofRandom( 0, 2*PI);
      float radius = 50;
      ofVec2f wanderTarget = ofVec2f(radius*cos(angle), radius*sin(angle));
      wanderTarget += target;
      ofVec2f desired = wanderTarget - location;  // A vector pointing from the location to the target
